...
- **智能应用切换**：一键切换常用应用程序（Chrome、Telegram、Notion、Zen、Spotify、微信）
- **高级窗口管理**：窗口置顶、透明度控制、半透明穿透、浏览器导航
- **智能主题切换**：基于日出日落时间自动切换系统主题
- **优化桌面时钟**：美观的桌面时钟显示，支持主题自适应
- **全局媒体控制**：Spotify等媒体播放控制
- **智能模拟输入**：支持暂停/继续的文本输入功能
- **高级连点器**：鼠标连点录制、播放和速度调节
- **键盘锁定**：临时锁定键盘输入，双击解锁
...
- **权限要求**：需要管理员权限运行（脚本会自动请求）
- **网络连接**：自动主题切换功能需要网络连接
- **性能优化**：已移除日志功能，减少系统资源占用
...
| `Alt+[` | 浏览器后退 | 映射到 Alt+Left（Chrome专用） |
| `Alt+]` | 浏览器前进 | 映射到 Alt+Right（Chrome专用） |
...
- **显示优化**：增加窗口宽度，确保时间完全显示
- **对齐改进**：使用VCenter居中对齐，提升视觉效果
...
- **智能输入**：支持复杂文本的逐字符模拟输入
- **速度控制**：提供正常速度和慢速两种模式
- **实时控制**：输入过程中可暂停/继续/退出（Space暂停，Escape退出）
- **进度显示**：显示输入进度和状态
- **体验优化**：改进暂停/继续控制逻辑，提升用户体验
- **使用场景**：适用于需要模拟人工输入的场景
...
**热键限制设置：**
```autohotkey
A_MaxHotkeysPerInterval := 200  ; 最大热键数量（防止性能问题）
A_HotkeyInterval := 2000        ; 时间间隔（毫秒）
```
